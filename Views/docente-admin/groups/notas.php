    ...
    <div class="main-body">
          ...
          <nav aria-label="breadcrumb" class="main-breadcrumb">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item"><a href="../groups/mostrar">Sección y Notas</a></li>
...
$id = $_GET['id'];
$sql= "SELECT seccion.idsec, seccion.nomsec, subgrade.idsub, subgrade.nomsub, teachers.idtea, teachers.dnite, teachers.nomte, teachers.correo, teachers.telet, teachers.usuario,teachers.foto,teachers.sexte, seccion.capa, seccion.state, GROUP_CONCAT(degree.iddeg, '..', degree.nomgra, '..' SEPARATOR '__') AS degree FROM seccion INNER JOIN subgrade ON seccion.idsub =subgrade.idsub INNER JOIN teachers ON seccion.idtea = teachers.idtea INNER JOIN degree ON subgrade.iddeg =degree.iddeg  WHERE idsec= '$id'  GROUP BY seccion.idsec"; 
...
<?php endforeach; ?>
    ...
    margin-top: 14px;
    margin-bottom: 0px;">
            <strong>No hay alumnos para esta Sección y Notas!</strong>
        </div>
    <?php endif; ?> 
...
<script> function verificarContactoDuplicado(contacto) {
  MongoClient.connect(url, function(err, client) {
    if (err) throw err;
    const db = client.db(config.php);
    const collection = db.collection('./Config/Config.php');

    // Verificar si el contacto ya existe en la base de datos
    collection.findOne({ email: contacto.email }, function(err, result) {
      if (err) throw err;
      if (result) {
        console.log('El contacto ya existe en la base de datos');
        // Aquí puedes manejar la lógica para no agregar el contacto duplicado
      } else {
        console.log('El contacto no existe en la base de datos');
        // Aquí puedes agregar el contacto a la base de datos
      }
      client.close();
    });
  });
